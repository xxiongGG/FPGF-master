...
for edge in edges:
    ...
    rate_r = count_r / count_all if count_all != 0 else 0
    rate_w = count_w / count_all if count_all != 0 else 0
    # rate_r = (count_r / count_all if count_all != 0 else 0) * (count_p_1 / count_num * 100)
    # rate_w = (count_w / count_all if count_all != 0 else 0) * (count_p_0 / count_num * 100)

    rate = rate_w * rate_r
    edge.append(rate)
    edge_directed.append(edge)

edge_final = []
edge_final_map = []
for i in range(int(len(edge_directed) / 2)):
    if edge_directed[i][2] > edge_directed[int(len(edge_directed) / 2) + i][2]:
        edge_final.append(edge_directed[i])

    else:
        edge_final.append(edge_directed[int(len(edge_directed) / 2) + i])
